-- Fix the create_internal_trip_v2 function to add the proper return value
-- The function needs to return a value, not void
...
-- Recreate with the correct return type (UUID)
CREATE OR REPLACE FUNCTION public.create_internal_trip_v2(
  trip_data json,
  wagon_id_param uuid
)
RETURNS uuid  -- Return the trip_id instead of void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
...
VALUES (
  ...
  gen_random_uuid(), 
  jsonb_build_object(
    'message', 'Fixed create_internal_trip_v2 function signature to return trip_id',
    'update_date', NOW()
  )
);
