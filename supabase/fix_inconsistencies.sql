...
VALUES ('UPDATE', 'system', NULL, 'Starting data consistency checks and fixes');
...
BEGIN
    ...
    IF wagon_inconsistencies = 0 AND trip_inconsistencies = 0 AND wagons_without_events = 0 THEN
        ...
        -- Log success
        INSERT INTO audit_logs (action, table_name, record_id, details)
        VALUES ('UPDATE', 'system', NULL, 'Data consistency checks passed. Database is in a consistent state.');
    ELSE
        ...
        VALUES ('WARNING', 'system', NULL, 
                'Some inconsistencies remain: ' || wagon_inconsistencies || ' wagon position issues, ' ||
                trip_inconsistencies || ' trip issues, ' || wagons_without_events || ' wagons without events');
    END IF;
END $$;
