-- ensure_complete_event_history_fixed2.sql
...
VALUES ('UPDATE', 'system', NULL, '{"message": "Ensuring complete event history for all wagons"}');
...
BEGIN
    ...
    LOOP
        ...
        -- Fixed ambiguous column reference by fully qualifying the column
        SELECT COUNT(*) INTO event_count 
        FROM wagon_trajectories wt
        WHERE wt.wagon_id = wagon_rec.id;
        ...
        IF NOT has_events AND wagon_rec.current_track_id IS NOT NULL THEN
            ...
            -- Log the creation with proper JSON formatting
            INSERT INTO audit_logs (action, table_name, record_id, details)
            VALUES ('INSERT', 'wagon_trajectories', wagon_rec.id::TEXT, 
                    json_build_object('message', 'Created initialization event for wagon', 'wagon_number', wagon_rec.number));
        END IF;
...
-- Update the database schema version with proper JSON formatting
INSERT INTO audit_logs (action, table_name, record_id, details)
VALUES ('UPDATE', 'system', NULL, '{"message": "Complete event history ensured for all wagons"}');
