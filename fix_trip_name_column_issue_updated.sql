...
DROP FUNCTION IF EXISTS public.fix_trip_trajectory_inconsistencies();
...
BEGIN
    ...
    FOR trip_rec IN 
        SELECT 
            t.id AS trip_id,
            -- Replace t.name with a COALESCE with transport_plan_number or trip ID
            COALESCE(t.transport_plan_number, 'Trip ' || t.id::text) AS trip_identifier,
            t.datetime,
            t.dest_track_id,
            t.type
        FROM 
