-- First drop any existing functions with the same names
DROP FUNCTION IF EXISTS get_wagons_at_time(timestamp with time zone);
DROP FUNCTION IF EXISTS get_track_wagons_at_time(uuid, timestamp with time zone);
DROP FUNCTION IF EXISTS get_track_occupancy_at_time(uuid, timestamp with time zone);
